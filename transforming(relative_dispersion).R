...
the_EWs <- function (dataset=original_data, window_size=50, steps=1){ #the width of rolling window is 50.
  ...
  for(j in window_indices) {
    ...
    relative_dispersion <-function(x) {sd(x) / sd(diff(x)[-1])} # load the "relative_dispersion" in  in Vest R package, link: https://github.com/vcerqueira/vest/blob/master/R/statistics.r
...
#step 3, compute a Earlier warming signal (e.g. relative_dispersion) 
results=the_EWs(dataset=original_data, window_size=50, steps=1)

#plot results
plot(results, type="o", ylab="Relative dispersion in each time window", col="blue")
